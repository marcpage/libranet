...
content id, which is also how fetches are made. Steps 1 and 2 wait for their
responses; the requests of each later step are pipelined.

The peer's public key goes straight into the source of truth once it is
known to hash to the peer's id, rather than through the validator, since
...
# The most a public key sent compressed may expand to. Node keys are a few
# hundred bytes, so this only stops a small body from expanding without bound.
_MAX_PUBLIC_KEY_BYTES: Final = 64 * 1024
...
class PeerExchange:
    ...
    def _hold_public_key(self, node_id: ContentId, response: PeerResponse, endpoint: str) -> None:
        """Store the public key ``response`` carries for ``node_id``.

        A key sent compressed is stored decompressed, as keys are checked
        against their id as they are stored (HttpApi §8).

        Raises:
            PeerAuthenticationError: ``response`` does not carry that key, or
                what hashes to ``node_id`` is not a public key.
        """
        key: bytes | None = response.body

        if response.status == HTTPStatus.OK and not node_id.matches(response.body):
            key = _decompressed(response.body, _MAX_PUBLIC_KEY_BYTES)

        if response.status != HTTPStatus.OK or key is None or not node_id.matches(key):
            raise PeerAuthenticationError(
                f"{endpoint} did not send its public key {node_id} (status {response.status})"
            )

        # Anything hashing to the id is that id's content, but only a key
        # belongs here: nothing else should reach the source of truth unannounced.
        try:
            decode_public_key(key)

        except KeyFileError as error:
            raise PeerAuthenticationError(
                f"{endpoint}'s id {node_id} is not a key: {error}"
            ) from error

        self._source_of_truth.write(node_id, key)
