...
def test_a_compressed_public_key_is_held_decompressed(
    exchange: PeerExchange, tmp_path: Path, config: LibranetConfig
) -> None:
    peer = FixturePeer(tmp_path / "compressing")
    peer.store.write(peer.identity.node_id, compress(peer.identity.public_key))

    try:
        exchange.open(peer.endpoint).close()

    finally:
        peer.stop()

    held = source_of_truth_store(config.storage).read(peer.identity.node_id)
    assert held == peer.identity.public_key
...
def test_a_peer_whose_id_is_not_a_key_is_refused(
    ...
    with raises(PeerAuthenticationError, match="is not a key"):
        exchange.open(peer.endpoint)
